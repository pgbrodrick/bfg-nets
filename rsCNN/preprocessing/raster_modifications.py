...
def add_edges_to_binary_image(input_file: str, output_file: str, binary_values: List =[0,1]):
    ...
    driver.Register()
    outDataset = driver.Create(output_file,
    ...
    binary_data[np.logical_and(binary_data == binary_values[0], signal.convolve2d((binary_data == binary_values[1]).astype(int),np.ones((3,3)),mode='same',fillvalue=0)).astype(bool)] = 2
