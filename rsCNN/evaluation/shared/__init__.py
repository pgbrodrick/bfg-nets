from rsCNN.evaluation.shared.colormaps import COLORMAP_CATEGORICAL, COLORMAP_ERROR, COLORMAP_METRICS, COLORMAP_WEIGHTS
from rsCNN.evaluation.shared.figures \
    import get_figure_and_grid, get_axis_iterator_for_sample_row, \
    plot_figures_iterating_through_samples_features_responses
from rsCNN.evaluation.shared.subplots \
    import plot_raw_features, plot_transformed_features, plot_raw_responses, plot_transformed_responses, \
    plot_raw_predictions, plot_transformed_predictions, plot_max_likelihood, plot_weights, plot_binary_error, \
    plot_raw_error_regression, plot_transformed_error_regression, add_internal_window_to_subplot
