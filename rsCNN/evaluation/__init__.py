...
class ExperimentReport(object):
    ...
    def create_report(self):
        ...
        with PdfPages(filepath_report) as pdf:
            ...
            #for fig in results.single_sequence_prediction_histogram(self.test_sequence, self.experiment, 'Training'):
            #    pdf.savefig(fig, bbox_inches='tight')
            ## Plot Validation Sequence
            #for fig in results.single_sequence_prediction_histogram(self.test_sequence, self.experiment, 'Validation'):
            #    pdf.savefig(fig, bbox_inches='tight')
