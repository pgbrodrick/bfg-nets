...
def build_or_load_rawfile_data(config, rebuild=False):
    ...
    data_container.feature_raw_band_types, = get_band_types(
        config.raw_feature_file_list, config.feature_raw_band_type_input)
    data_container.response_raw_band_types, = get_band_types(
...
class Dataset:
    ...
    def build_or_load_scalers(data_container, rebuild=False):
        ...
        self.train_folds=[x for x in range(
            config.n_folds) if x is not config.validation_fold and x is not config.test_fold]
