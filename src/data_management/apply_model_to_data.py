...
def apply_model_to_raster(cnn, data_config, feature_file, destination_basename, make_png=False, make_tif=True, feature_transformer=None, response_transformer=None)
    ...
    for col in collist:
      images = []
      for n in rowlist:
        d = feature[n-data_config.window_radius:n+data_config.window_radius,col-data_config.window_radius:col+data_config.window_radius].copy()
        ...
        for n in rowlist:
          ...
          if (data_config.internal_window_radius < data_config.window_radius):
            mm = rint(data_config.window_radius - data_config.internal_window_radius)
            p = p[mm:-mm,mm:-mm,:]
      ...
      if (make_tif):
        ...
        outDataset = driver.Create(destination_basename + '.tif',output.shape[1],output.shape[0],cnn.n_classes,gdal.GDT_Float32)
