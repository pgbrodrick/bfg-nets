...
def test_experiment_asserts_error_on_initialization_with_conflicting_config(config_new) -> None:
    experiment_a = experiments.Experiment(config_new)
    config = config_new
...
def test_experiment_build_or_load_model_builds_new_model_successfully(config_new) -> None:
    experiment = experiments.Experiment(config_new)
...
def test_experiment_build_or_load_model_loads_existing_model_successfully(config_new) -> None:
    experiment_a = experiments.Experiment(config_new)
...
def test_experiment__load_existing_history_loads_sets_attrs_with_val_loss(config_new) -> None:
    experiment = experiments.Experiment(config_new)
    expected_history = {'val_loss': [1, 1, 0, 1], 'lr': [1, 1, 0, 1], experiments.KEY_HISTORY_IS_MODEL_TRAINED: True}
...
def test_experiment__load_existing_history_loads_sets_attrs_no_val_loss(config_new) -> None:
    experiment = experiments.Experiment(config_new)
    expected_history = {'lr': [1, 1, 0, 1], experiments.KEY_HISTORY_IS_MODEL_TRAINED: False}
    histories.save_history(expected_history, experiment.filepath_history)
    experiment._build_new_model((3, 3, 2))
    experiment._load_existing_history()
    assert experiment.history == expected_history
...
def test_experiment_fit_model_with_sequences_raises_on_existing_model_no_resume(config_new) -> None:
    experiment_a = experiments.Experiment(config_new)
...
def test_experiment_fit_model_with_data_container_does_everything(config_new, monkeypatch) -> None:
    experiment = experiments.Experiment(config_new)
...
def test_experiment_fit_model_with_sequences_does_everything(config_new, monkeypatch) -> None:
    experiment = experiments.Experiment(config_new)
    experiment.build_or_load_model(num_features=1)
...
def test_load_experiment_from_directory_returns_experiment(config_new) -> None:
    experiment_a = experiments.Experiment(config_new)
    experiment_b = experiments.load_experiment_from_directory(config_new.model_training.dir_out)
    assert type(experiment_b) is experiments.Experiment
