...
class Reporter(object):
    ...
    def __init__(
        ...
        errors = config.get_human_readable_config_errors(
            include_sections=["model_reporting"]
        )
    ...
    def create_model_report(self) -> None:
        filepath_report = os.path.join(
            self.experiment.config.model_training.dir_out, _FILENAME_MODEL_REPORT
        )
        with PdfPages(filepath_report) as pdf:
            ...
            if "R" in self.data_container.response_band_types:
                self._add_figures(
                    self.plot_regression_deviation(sampled, validation_sampled), pdf
                )
    ...
    def _create_model_report_for_sequence(
        self, sampled: samples.Samples, pdf: PdfPages
    ) -> None:
        if (
            self.experiment.is_model_trained
            and self._get_response_data_types() is _LABEL_CATEGORICAL
        ):
        ...
        if self.experiment.config.model_reporting.network_progression_show_full:
            self._add_figures(
                self.plot_network_feature_progression(sampled, compact=False), pdf
            )
        if self.experiment.config.model_reporting.network_progression_show_compact:
            self._add_figures(
                self.plot_network_feature_progression(sampled, compact=True), pdf
            )
        self._add_figures(self.plot_spatial_error(sampled), pdf)

    def _add_figures(
        self, figures: List[plt.Figure], pdf: PdfPages, tight: bool = True
    ) -> None:
        for fig in figures:
    ...
    def plot_log_warnings_and_errors(self) -> List[plt.Figure]:
        return logs.plot_log_warnings_and_errors(
            self.data_container.config, self.experiment.config
        )
    ...
    def plot_network_feature_progression(
        ...
        return networks.plot_network_feature_progression(
            ...
            compact=compact,
            max_pages=max_pages
            or self.experiment.config.model_reporting.network_progression_max_pages,
            max_filters=max_filters
            or self.experiment.config.model_reporting.network_progression_max_filters,
        )
    ...
    def plot_samples(
        ...
        max_responses_per_page = (
            max_responses_per_page
            or self.experiment.config.model_reporting.max_responses_per_page
        )
        return plotter(
            sampled,
            max_pages=max_pages
            or self.experiment.config.model_reporting.max_pages_per_figure,
            max_samples_per_page=max_samples_per_page
            or self.experiment.config.model_reporting.max_samples_per_page,
            max_features_per_page=max_features_per_page
            or self.experiment.config.model_reporting.max_features_per_page,
    ...
    def plot_sample_histograms(
        ...
        return samples_viz.plot_sample_histograms(
            sampled, max_responses_per_page=max_responses_per_page
        )
        ...
        for _r in range(len(self.data_container.response_band_types)):
            ...
            if self.data_container.response_band_types[_r] == "R":
                bounds = self.parity_plot(
                    train_sampled.raw_predictions,
                    train_sampled.raw_responses,
                    plt.subplot(gs1[0, 0]),
                    _r,
                )
    ...
    def parity_plot(
        ...
        slope, intercept, r_value, p_value, std_err = stats.linregress(
            np.squeeze(test_Y), np.squeeze(pred_Y)
        )
        ...
        r2o = str(
            round(
                1
                - np.sum(np.power(test_Y - pred_Y, 2))
                / (np.sum(np.power(test_Y - np.mean(pred_Y), 2))),
                3,
            )
        )
        ...
        if len(bounds) == 0:
            ...
            pmin = bounds[0]
            pmax = bounds[1]
        z, xrange, yrange = np.histogram2d(
            test_Y, pred_Y, bins=bins, range=[[pmin, pmax], [pmin, pmax]]
        )
    ...
    def _get_response_data_types(self) -> str:
        data_types = set(
            [
                dt
                for file_dts in self.experiment.config.raw_files.response_data_type
                for dt in file_dts
            ]
        )
        if data_types == {"C"}:
            ...
            raise AssertionError(
                "Reporter does not currently support mixed response data types."
            )
        else:
