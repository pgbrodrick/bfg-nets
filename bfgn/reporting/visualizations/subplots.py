...
def _plot_sample_attribute(
    ...
    if attribute_name != _LABEL_CATEGORICAL_RESPONSES:
        ...
        assert not colormaps.check_is_categorical_colormap_repeated(
            sampled.num_responses
...
def plot_classification_predictions_max_likelihood(
    ...
    ax.imshow(
        np.argmax(sampled.raw_predictions[idx_sample, ...], axis=-1),
        vmin=min_,
        vmax=max_,
...
def plot_binary_error_classification(
    ...
    if sampled.raw_responses is None or sampled.raw_predictions is None:
        _logger.debug(
            "Not plotting binary misclassifications; "
...
def plot_raw_error_regression(
    ...
    if sampled.raw_responses is None or sampled.raw_predictions is None:
        _logger.debug(
            "Not plotting raw regression residuals; "
            + "raw_responses and/or raw_predictions attributes are not available"
        )
        return
...
def plot_transformed_error_regression(
    ...
    if sampled.raw_responses is None or sampled.raw_predictions is None:
        _logger.debug(
            "Not plotting transformed regression residuals; "
            + "raw_responses and/or raw_predictions attributes are not available"
        )
        return

    error = (
        sampled.trans_predictions[idx_sample, :, :, idx_response]
