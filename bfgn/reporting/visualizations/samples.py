...
def _plot_samples(
    ...
    for idx_page in range(num_pages):
        ...
        idxs_samples = range(
            idx_page * max_samples_per_page, (1 + idx_page) * max_samples_per_page
        )
        for idx_sample in idxs_samples:
            sample_axes = iter(
                [
                    plt.subplot(grid[idx_sample, idx_subplot])
                    for idx_subplot in range(num_subplots)
                ]
            )
            sample_plotter(
                sampled, idx_sample, num_features, num_responses, sample_axes
            )
        fig.suptitle(
            "{}Sequence Samples (page {})".format(
...
def _plot_classification_sample(
    ...
    if sampled.raw_predictions is not None:
        ...
        num_subplots += 1
        subplots.plot_binary_error_classification(
            sampled, idx_sample, sample_axes.__next__(), idx_sample == 0, False
        )
...
def plot_sample_histograms(sampled: samples.Samples, max_responses_per_page: int = 15):
    ...
    while _response_ind < sampled.num_responses:
        ...
        for _r in range(
            _response_ind,
            min(_response_ind + max_responses_per_page, sampled.num_responses),
        ):
            ...
            if sampled.trans_predictions is not None:
                b, h = _get_lhist(
                    sampled.trans_predictions[..., _r], hist_range=hist_range
                )
            ...
            if sampled.raw_predictions is not None:
                b, h = _get_lhist(
                    sampled.raw_predictions[..., _r], hist_range=hist_range
                )
                ax.plot(h + (h[1] - h[0]) / 20.0, b, color="black")
        ...
        fig_list.append(fig)
        fig.suptitle(
            "{} Sequence Response Histogram (page {})".format(
                sampled.data_sequence_label, len(fig_list)
            )
        )
    return fig_list
